...
| Op fixtures, result | 7 of the 8 match at 1e-5 absolute / 1e-4 relative; the composite latent MoE block at 2e-4 absolute with cosine 1.000000, because it chains six matmuls through the MXFP4 decode path | `lazy_lora/tests/test_reference_ops.py` — `ABS_TOL = 1e-5`, `REL_TOL = 1e-4`, and `abs_tol=2e-4` hardcoded for the `moe` case, widened in commit `91964c6`; `Bulgular.md` §15.6 |
...
| Threshold | TR news ≤ 0.441 (−3 %) **and** EN Wikipedia ≤ 0.198 (+2 % at most) | `DEVAM.md` §16.1, fixed in commit `85af2a8` |
| Pre-registration | commit `85af2a8`, 8 Sep 2026 07:54:49 +0300, **29 h** before the run started at 9 Sep 12:53:33; annotated tag `preregistration-2026-09-08` on that commit | `git log 85af2a8`, `git show preregistration-2026-09-08`, `evidence/run_manifest.json` (`started=1788947613.66`) |
