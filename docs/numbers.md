...
| Token count vs step time (layers 0-11, 6 Sep 2026) | 1024 tokens: 1602 s, 176.2 GB read, RSS 3.3 GB; 2048 tokens: 3107.7 s, 196.7 GB read, RSS 5.28 GB — 1.94× time for 1.12× bytes, so compute-bound beyond ~1024 tokens | workspace `traces/profile_{1024,2048}_native_clean_2026-09-06/trace.json` (not in evidence/; add on request) |
| USB share of the aggregate read rate | ≈ 102 MB/s of the 110 MB/s aggregate: the NVMe trunk is 108.8 GB × 2 sweeps ≈ 218 GB per step over 26,784 s ≈ 8 MB/s; 102/115 ≈ 89 % of the enclosure benchmark, so prefetch can recover at most ~10 % | derived from the rows above; not a measurement |
| Disk workload of the run | 110 MB/s × 26,784 s ≈ 2.95 TB read per step, ≈ 295 TB over 100 steps, on a consumer 2 TB drive; typical consumer workload ratings are tens of TB per year | derived; SMART wear counters (reallocated, pending, uncorrectable, CRC) all 0 on 10 Sep 2026 and now alerted by the watchdog |
