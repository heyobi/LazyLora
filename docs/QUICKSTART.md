...
| Step 1 says `SKIP: fixtures absent` | Expected. The op fixtures live in the reference repository: `git clone https://github.com/FareedKhan-dev/kimi-k3-in-c ../kimi-k3-in-c`, or point `LAZYLORA_REF_FIXTURES` at its `tests/fixtures/ops`. Everything else still runs. |
